...
# Load the dataset
data_path = 'owid-covid-data.csv'  # Adjusted the path
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'new_cases.png'))
print("Plot saved: new_cases.png")
plt.close()

# Plot vaccination progress over time for each country
plt.figure(figsize=(12, 6))
for country in countries_of_interest:
    country_data = covid_df[covid_df['location'] == country]
    plt.plot(country_data['date'], country_data['total_vaccinations'], label=country)

plt.title('Total Vaccinations Over Time in Selected Countries')
plt.xlabel('Date')
...
plt.xticks(rotation=45)
plt.tight_layout()
plt.savefig(os.path.join(output_dir, 'vaccinations.png'))
print("Plot saved: vaccinations.png")
plt.close()
